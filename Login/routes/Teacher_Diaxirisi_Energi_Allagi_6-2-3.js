...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await collection.updateOne(
      { _id: my_objectId, katastasi: "Ενεργή" }, // Εύρεση της διπλωματικής με βάση το ID και την κατάσταση
      {
        $set: {
          katastasi: "υπό εξέταση", // Ενημέρωση της κατάστασης
        },
        $push: {
          proigoumenesKatastaseis: "Ενεργή", // Προσθήκη του αριθμού 3 στο array για δήλωση της προηγούμενης κατάστασης "υπό εξέταση"   Το ΞΑΝΑΒΛΕΠΟΥΜΕ
        },
      }
    );
    ...
    if (result.matchedCount === 0) {
      ...
      res.status(200).json({ message: `✅ Ενημερώθηκε η διπλωματική με id: ${id} σε "υπό εξέταση".` });
    }
  } catch (error) {
